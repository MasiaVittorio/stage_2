...
class SelectSnackbar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(
      builder: (_, constraints) => _SnackbarSelector(
...
class _SnackbarSelectorState extends State<_SnackbarSelector> with SingleTickerProviderStateMixin {
  ...
  double get initialScrollOffset {
    ...
    if (max <= 0) return 0;
    return (elementWidth *
                ( //scroll the selected one at the top
                    (widget.initialIndex ?? 0.0) + 1 //another step to make it disappear
                ) -
  ...
  Widget build(BuildContext context) {
    ...
    return AnimatedBuilder(
      ...
      builder: (_, child) => Transform.translate(
        offset: Offset(
          (1 - animationController.value) * (rightAligned ? 200 : -200),
          0,
        ),
        child: child,
      ),
