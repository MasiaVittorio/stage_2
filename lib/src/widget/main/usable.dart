...
class Stage<T, S> extends StatelessWidget {
  ...
  const Stage({
    ...
    /// Either you put this, or you set the initial controller values
    ...
    /// required (if not external controller)
    this.panelPages,
  ...
  /// Set if you want to save user changes to disk
  final T Function(dynamic)? jsonToMainPage;

  /// All these are optional if the pages are of an already jsonable type
  ...
  final StagePanelDecorationBuilder?
      customDecorationBuilder; // To override radiuses, background color, and what have you
  ...
  static BoxShadow _defaultShadowBuilder(double panelVal, StageColorPlace place) => BoxShadow(
        ...
        // ),
        blurRadius: panelVal.mapToRangeLoose(
          //the value is provided already clamped
          place.isTexts ? 4.0 : 8.0,
          place.isTexts ? 8.0 : 14.0,
        ),
