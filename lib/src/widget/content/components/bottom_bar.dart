...
class _BottomBar<T, S> extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return data.mainPagesController._orderedPages.build(
      ((_, orderedPages) => data.dimensionsController.dimensions.build(((_, dimensions) => data
          .themeController.derived.mainPageToPrimaryColor
          .build(((_, primaryColorsMap) => data.themeController.derived._mainPrimaryColor.build(
              ((_, color) => data.themeController.derived.themeData.build((_, theme) => data
                  .mainPagesController._enabledPages
                  .build(((_, enabled) => data.themeController.colorPlace.build(((_, place) => data.mainPagesController._page.build(((_, page) => data.badgesController.mainPages.build((_, badges) {
                        final bool googleLike = place.isTexts;

                        final bool single = primaryColorsMap == null;
                        final Color singleBackground = color;
                        final bool useAccent = single && data.themeController.accentSelectedPage;
