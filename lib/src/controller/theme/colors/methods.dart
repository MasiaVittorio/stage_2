...
extension StageColorsMethods<T, S> on _StageColorsData<T, S> {
  //============================================
  // Dark Primary Colors
  bool _editDarkMainPageToPrimary(
      {required T page, required DarkStyle? style, required Color color}) {
  ...
  bool _editDarkPanelPageToPrimary(
      {required S page, required DarkStyle style, required Color color}) {
  ...
  bool _enableDarkMainPagedColors() {
    ...
    darkMainPageToPrimaries.update(
      _cachedDarkMainPageToPrimaries.value ??
  ...
  bool _disableDarkMainPagedColors() {
    if (darkMainPageToPrimaries.value == null) return false;
    _cachedDarkMainPageToPrimaries.update(
      darkMainPageToPrimaries.value,
    );
    darkMainPageToPrimaries.update(null);
    return true;
  }

  bool _enableDarkPanelPagedColors() {
    if (darkPanelPageToPrimaries.value != null) return false;
    darkPanelPageToPrimaries.update(
      _cachedDarkPanelPageToPrimaries.value ??
  ...
  bool _disableDarkPanelPagedColors() {
    if (darkPanelPageToPrimaries.value == null) return false;
    _cachedDarkPanelPageToPrimaries.update(
      darkPanelPageToPrimaries.value,
    );
    darkPanelPageToPrimaries.update(null);
    return true;
  }
  ...
  bool _editLightMainPrimary(Color color) => lightMainPrimary.update(color);
  ...
  bool _editLightPanelPrimary(Color color) => lightPanelPrimary.update(color);

  bool _enableLightMainPagedColors() {
    if (lightMainPageToPrimary.value != null) return false;
    lightMainPageToPrimary.update(
      _cachedLightMainPageToPrimary.value ??
          <T, Color>{
  ...
  bool _disableLightMainPagedColors() {
    if (lightMainPageToPrimary.value == null) return false;
    _cachedLightMainPageToPrimary.update(
      lightMainPageToPrimary.value,
    );
    lightMainPageToPrimary.update(null);
    return true;
  }

  bool _enableLightPanelPagedColors() {
    if (lightPanelPageToPrimary.value != null) return false;
    lightPanelPageToPrimary.update(
      _cachedLightPanelPageToPrimary.value ??
          <S, Color>{
            for (final key in parent.parent.panelPagesController!.pagesData.keys)
  ...
  bool _disableLightPanelPagedColors() {
    if (lightPanelPageToPrimary.value == null) return false;
    _cachedLightPanelPageToPrimary.update(
      lightPanelPageToPrimary.value,
    );
    lightPanelPageToPrimary.update(null);
    return true;
  }

  //============================================
  // Light Accent Color
  bool _editLightAccent(Color color) => lightAccent.update(color, distinct: true);
