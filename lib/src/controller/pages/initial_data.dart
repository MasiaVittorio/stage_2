...
class StagePagesData<T> {
  ...
  Set<T> get _allKeys => <T>{
        if (orderedPages != null) ...orderedPages!,
        if (enabledPages != null) ...enabledPages!.keys,
        if (pagesData != null) ...pagesData!.keys,
      };

  StagePagesData<T> fillWith(StagePagesData<T>? other) {
    final Set<T> keys = <T>{
      ..._allKeys,
      if (other != null) ...other._allKeys,
    };
  ...
  //================================
  // Static
  static Map<T, bool> _getEnabled<T>(
      Map<T, bool?>? starting, Map<T, StagePage?> pagesData, List<T>? orderedPages) {
    return <T, bool>{
      for (final T key in <T>{
        ...pagesData.keys,
        if (starting != null) ...starting.keys,
        if (orderedPages != null) ...orderedPages
      })
        key: (starting == null ? null : starting[key]) ?? true,
    };
  }
}
