...
class StageDimensions {
  ...
  /// 1.0 makes the body move like the panel, 0.1 is recommended
  ...
  const StageDimensions({
    ...
    bool? forceOpenedPanelOverNavBar = true,
        ...
        panelHorizontalPaddingClosed =
            panelHorizontalPaddingClosed ?? defaultPanelHorizontalPaddingClosed,
  ...
  StageDimensions copyWith({
    ...
    double? parallax,
      ...
      StageDimensions(
        ...
        panelHorizontalPaddingClosed:
            panelHorizontalPaddingClosed ?? this.panelHorizontalPaddingClosed,
        panelHorizontalPaddingOpened:
            panelHorizontalPaddingOpened ?? this.panelHorizontalPaddingOpened,
  ...
  //=============================
  // Getters
  double bottomBarSize(MediaQueryData mediaQuery) =>
      barSize + collapsedPanelSize / 2 + RadioNavBar.bottomPaddingFromMQ(mediaQuery);
}
...
class _StageDerivedDimensions {
  _StageDerivedDimensions({
    ...
    panelMinBottomPosition = bottomBarSize // away from bottom
        -
        base.collapsedPanelSize / 2 // but a bit of overlap
        -
        maxDownExpansion; // and more overlap if needed

    totalPanelHeight = constraints.maxHeight // total height
        -
    ...
    panelMinBottomPositionKeyboard =
        panelMinBottomPosition + mediaQuery.viewInsets.bottom; // Keyboard

    alertHeightClamp = (constraints.maxHeight // total height
            -
        ...
        .clamp(base.collapsedPanelSize, double.infinity);

    /// It cannot be smaller than the collapsed panel tho,
    /// so if the keyboard is fucking huge the alert will go higher than the screen, fuck it
  }

  late double panelWidthOpened;

  /// in general it can be opened and use this value even if it is just for an alert!
  late double panelWidthClosed;

  late double topPadding;

  /// System notification bar
  late double bottomBarSize;

  /// Colored part from the bottom of the screen
  late double minTopBarSize;

  /// Accounting for top padding when panel is closed
  late double extendedAppBarSize;

  /// When panel is opened (without accounting for top padding)
  late double maxTopBarSize;

  /// Accounting for top padding when panel is opened
  late double maxDownExpansion;

  /// How much the panel goes down if it does

  // static const double minimumBottomPadding = 16.0; /// If expanding the panel down, leave this space from the bottom
  double minimumBottomPadding(StageDimensions base) => base
      .panelHorizontalPaddingOpened; // If expanding the panel down, leave this space from the bottom

  late double panelMinBottomPosition;

  /// Accounts for the base position of the panel and the expansion ONLY (use it for the regular panel)
  late double totalPanelHeight;

  /// NOT in case of alert
  late double panelDelta;

  /// Difference between max and minimum dimension of the panel (NOT in case of alert)

  late double panelMinBottomPositionKeyboard;

  /// Accounts for the base position of the panel and the expansion AND THE KEYBOARD (use it only if alert is shown)
  late double alertHeightClamp;

  /// Maximum extent of an alert (accounts for keyboard)
